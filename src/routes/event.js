...
router.post('/addevent',upload.single('poster'),async(req,res,next)=>{
...
router.delete('/delevent/:id',async(req,res,next)=>{
    const {id} = req.params;
...
router.put('/updateevent/:id',upload.array('images',5),async(req,res,next)=>{
    const {id} = req.params;
    const images = req.files;
    const imagesUrl = [];
    images.forEach(async(image)=>{
        const imageUrl = await cloudinary(image.path);
